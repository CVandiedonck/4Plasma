...
plink -bfile input/ic_cases --make-bed --out data/ic_cases
plink -bfile input/ic_controls --make-bed --out data/ic_controls
plink --bfile input/ic_cases --exclude input/toexclude.txt --make-bed --out data/ic_cases_autosomes
plink --bfile input/ic_controls --bmerge data/ic_cases_autosomes.bed data/ic_cases_autosomes.bim data/ic_cases_autosomes.fam --make-bed --out data/t1dcc
plink --bfile input//ic_controls --exclude input/rs3754055.txt --make-bed --out input/ic_controls2
plink --bfile input/ic_cases_autosomes --exclude input/rs3754055.txt --make-bed --out input/ic_cases_autosomes2
plink --bfile input/ic_controls --flip data/t1dcc-merge.missnp --make-bed --out data/ic_controls_flipped
plink --bfile input/ic_controls_flipped --bmerge input/ic_cases_autosomes2.bed input/ic_cases_autosomes2.bim input/ic_cases_autosomes2.fam --make-bed --out data/t1dcc

# clean SNPs
plink --bfile input/t1d_ctrl --geno 0.05 --make-bed --out output/filter1
...
# ou NEW : enlevez les individus apparent�s avec:
...
dev.off()
png("ouptut/qqplot.png")
qq(gwasResults$P, main="Q-Qplot of P-values")
dev.off()
quit('no')

# perform MDS analysis
plink --bfile output/filter6 --exclude data/mhc_range.txt --range --make-bed --out output/withoutmhc
...
# Start R
mds <- read.table("output/stratifwithoutldpc.ped", header=F)
png("output/mds.png")
par(mfrow=c(2,2))
...
# trios analysis
plink --bfile data/ic_trio --tdt --adjust --out output/tdttrio
